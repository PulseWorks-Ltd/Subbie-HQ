...
export async function POST(request: Request, context: { params: { projectId: string } }) {
  ...
  // Hours on Site has no dedicated permission module of its own — every
  // one of its routes is gated on plain project access only (see
  // lib/hours-on-site.ts's callers), so a request targeting it skips the
  // Variations/Site Instructions module check below entirely rather than
  // forcing a module answer that doesn't apply.
  if (payload.hoursOnSiteSheetId) {
    const sheet = await prisma.hoursOnSiteSheet.findFirst({ where: { id: payload.hoursOnSiteSheetId, projectId } });
    if (!sheet) {
