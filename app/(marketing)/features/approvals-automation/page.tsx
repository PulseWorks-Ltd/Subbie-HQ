...
export const metadata: Metadata = {
  ...
  description:
    "Site Managers and Main Contractors approve via a secure link — no app, no login required — and, if you want, let it send itself on schedule."
};
...
export default function ApprovalsAutomationPage() {
  return (
    <FeaturePageView
      ...
      dek="Site Managers approve via a secure link — no app, no login required."
      ...
      helps={[
        "Send the Package with a secure link — no app to download, no account, no password, nothing for the Main Contractor or Site Manager to set up. They open it on their phone or laptop, see exactly what's being asked (the value, what's changed since the last request, the evidence behind it), and Approve, Reject, or Comment directly. The same mechanism works for Acknowledging an Update, approving a Dayworks or Hours on Site sheet, or getting something Signed off — always a recorded acknowledgement of what was sent, never presented as a legally binding electronic signature.",
        "Every request is tracked from send to response: pending, responded, or expired, right there on the item. No more wondering whether it was actually seen, or chasing someone to find a login they never set up.",
