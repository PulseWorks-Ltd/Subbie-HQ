      ...
      messages: [
        {
          ...
          content:
            ...
            "startTime/finishTime/task/weather/location: extract only if clearly legible on the sheet; null if not shown — their absence is normal and never blocks extraction. " +
            "date: the date recorded on the sheet, converted to an ISO 8601 date (YYYY-MM-DD) — e.g. '25.8.26', '25/08/2026', and '25 Aug 26' all mean 2026-08-25; use your best judgement on day/month order from NZ/AU date conventions (day before month) when it's ambiguous; null if no date is shown. " +
...
// "Generate QA Document" — drafts the covering email a compiled QA
// Document PDF is attached to. Same "draft, then human reviews before
// sending" rule as draftPaymentClaimEmail — nothing is sent until the
// user confirms (see the qa-documents send route).
export async function draftQaDocumentEmail(
  params: {
    projectName: string;
    docNumber: string; // already formatted, e.g. "QA-000123"
    recordCount: number;
    periodDescription: string; // e.g. "4 Sept 2026 to 12 Sept 2026"
    authorName: string;
  },
  usageContext: Omit<AiUsageContext, "feature">
): Promise<DraftedUpdateEmail> {
    ...
    messages: [
      {
        role: "system",
        content:
          "You draft a short, professional covering email a subcontractor sends to a Main Contractor or client along with an attached Quality Assurance Document PDF, on a New Zealand construction project. " +
          "Respond with only a JSON object matching this exact shape: " +
          '{"subject": string, "body": string}. ' +
          "subject: short and clear, naming the project and the QA document reference. " +
          "body: an appropriate greeting, one or two sentences stating this QA document compiles the site records for the stated period and how many records it contains, a note that it's attached as a PDF, a professional sign-off using the author's name. " +
          "Do not include placeholder brackets like [Your Name]. Do not restate every record in the email body — the PDF is the authoritative document; the email is just the covering note. Do not fabricate any detail not given below."
      },
      {
        role: "user",
        content: `Project: ${params.projectName}\nAuthor: ${params.authorName}\nQA Document reference: ${params.docNumber}\nRecords included: ${params.recordCount}\nPeriod covered: ${params.periodDescription}`
      }
    ]
  }, { ...usageContext, feature: "qa_document_email_draft" });
...
const ExtractedPaymentClaimBaselineVisionSchema = z.object({
  ...
  claimNumber: z.number().int().nullable(),
      ...
      messages: [
        {
          ...
          content:
            ...
            '{"claimReference": string | null, "claimNumber": number | null, "claimDate": string | null, "periodStart": string | null, "periodEnd": string | null, "projectNameOnDocument": string | null, "projectReferenceOnDocument": string | null, "contractReferenceOnDocument": string | null, "originalContractSum": number | null, "approvedVariationsTotal": number | null, "revisedContractSum": number | null, "pendingVariationsTotal": number | null, "grossClaimToDate": number | null, "retentionPercent": number | null, "retentionToDate": number | null, "netClaimToDate": number | null, "previousClaims": number | null, "currentClaim": number | null, "contractItems": [{"reference": string | null, "description": string, "contractValue": number | null, "revisedValue": number | null, "claimedPercentToDate": number | null, "claimedAmountToDate": number | null, "currentPeriodAmount": number | null, "previousClaimedAmount": number | null}], "variations": [{"reference": string | null, "description": string, "submittedValue": number | null, "approvedValue": number | null, "claimedPercentToDate": number | null, "claimedAmountToDate": number | null, "currentPeriodAmount": number | null, "approvalStatus": string | null}], "confidence": number, "notes": string | null}. ' +
            "claimReference: the claim's own number/label exactly as printed (e.g. 'Payment Claim #07', 'PC-07'), or null. " +
            "claimNumber: the plain integer sequence number of this claim within the project, parsed out of that same label if it clearly contains one (e.g. 'Payment Claim #07' -> 7, 'PC-07' -> 7, 'Progress Claim No. 10' -> 10) — null if no clear integer sequence number is discernible (a purely descriptive label like 'Final Claim' with no number, or genuine ambiguity). " +
